...
def Sdn_func(data_portrait_FT):
    """
    """
    data_port_FT = data_portrait_FT
    Sdn = np.sum(np.abs(data_port_FT) ** 2, axis=-1)
    return Sdn

def Spn(scattering_portrait_FT, model_portrait_FT):
    """
    """
    scat_port_FT = scattering_portrait_FT
    model_port_FT = model_portrait_FT
    Spn = np.sum(np.abs(scat_port_FT) ** 2 * np.abs(model_port_FT) ** 2, axis=-1)
    return Spn

def Spn_deriv(abs_scattering_portrait_FT_deriv, model_portrait_FT):
    """
    """
...
def Spn_2deriv(abs_scattering_portrait_FT_2deriv, model_portrait_FT):
    """
    """
...
def Cdpn(data_portrait_FT, model_portrait_FT, scattering_portrait_FT, phasor):
    """
    """
    ...
    model_port_FT = model_portrait_FT
    scat_port_FT = scattering_portrait_FT
    Cdpn = np.real(np.sum(data_port_FT * np.conj(model_port_FT) * \
                          np.conj(scat_port_FT) * phasor, axis=-1))
    return Cdpn

def Cdpn_deriv_phase_shifts(data_portrait_FT, model_portrait_FT,
    ...
    Cdpn_deriv_phase_shifts = np.real(np.sum(2.0j * np.pi * iharm * \
                                             data_port_FT * np.conj(model_port_FT) * np.conj(scat_port_FT) * \
                                             phasor, axis=-1))
    return Cdpn_deriv_phase_shifts


def Cdpn_2deriv_phase_shifts(data_portrait_FT, model_portrait_FT,
                             scattering_portrait_FT, phasor):
    """
    """
    data_port_FT = data_portrait_FT
    ...
    scat_port_FT = scattering_portrait_FT
    iharm = np.arange(phasor.shape[1])
    Cdpn_2deriv_phase_shifts = np.real(np.sum(pow(2.0j * np.pi * iharm, 2.0) * \
                                              data_port_FT * np.conj(model_port_FT) * np.conj(scat_port_FT) * \
                                              phasor, axis=-1))
    return Cdpn_2deriv_phase_shifts


def Cdpn_deriv(data_portrait_FT, model_portrait_FT,
               scattering_portrait_FT_deriv, phasor, phase_shifts_deriv,
               Cdpn_deriv_phase_shifts):
    """
    """
    data_port_FT = data_portrait_FT
    model_port_FT = model_portrait_FT
    ...
    iharm = np.arange(phasor.shape[1])
    Cdpn_deriv_phis = Cdpn_deriv_phase_shifts
    dphi, dDM, dGM = Cdpn_deriv_phis * phis_deriv
...
def Cdpn_2deriv(data_portrait_FT, model_portrait_FT,
                scattering_portrait_FT_deriv, scattering_portrait_FT_2deriv, phasor,
                phase_shifts_deriv, phase_shifts_2deriv, Cdpn_deriv_phase_shifts,
                Cdpn_2deriv_phase_shifts):
    """
    """
    data_port_FT = data_portrait_FT
    ...
    iharm = np.arange(nharm)
    Cdpn_deriv_phis = Cdpn_deriv_phase_shifts
    Cdpn_2deriv_phis = Cdpn_2deriv_phase_shifts
    Cdpn_hess_phis = (Cdpn_2deriv_phis * phis_deriv_matrix) + \
                     (Cdpn_deriv_phis * phis_2deriv)
    Cdpn_hess_scat = np.real(np.sum(data_port_FT * np.conj(model_port_FT) * \
                                    np.conj(scat_port_FT_2deriv) * phasor, axis=-1))
    Cdpn_hess_cross = np.zeros([nphase, nscat, nchan])
    Cdpn_cross = np.real(np.sum(2.0j * np.pi * iharm * data_port_FT * \
                                np.conj(model_port_FT) * np.conj(scat_port_FT_deriv) * phasor,
                                axis=-1))
    for iparam in range(nphase):
        for jparam in range(nscat):
            Cdpn_hess_cross[iparam, jparam] = phis_deriv[iparam] * \
                                              Cdpn_cross[jparam]
    hessian = np.zeros([nphase + nscat, nphase + nscat, nchan])
    hessian[:nphase, :nphase] = Cdpn_hess_phis
    hessian[nphase:, nphase:] = Cdpn_hess_scat
    hessian[:nphase, nphase:] = Cdpn_hess_cross
    hessian[nphase:, :nphase] = Cdpn_hess_cross.transpose(1, 0, 2)
    return hessian
...
def fit_portrait_full_function(params, data_portrait_FT, model_portrait_FT,
    ...
    #print("Number of harmonics:", nharm)
    #print("Data portrait (FT) shape:", data_port_FT.shape[0])
    nchan = data_portrait_FT.shape[0]
    nbin = 2 * (nharm - 1)
    #print("Number of bins:", nbin)
    phis = phase_shifts(phi, DM, GM, freqs, nu_DM, nu_GM, P, mod=False)
    #print("Phase shifts:", phis)
    phsr = phasor(phis, nharm)
    #print("Phasor:", phsr)
    taus = scattering_times(tau, alpha, freqs, nu_tau)
    #print("Scattering times:", taus)
    scat_port_FT = scattering_portrait_FT(taus, nbin, binshift=binshift)
    #print("Scattering portrait (FT):", scat_port_FT)
    #print('Data portrait (FT):', data_port_FT)
    #print('Absolute values (FT):', np.abs(data_port_FT))
    Sdn = Sdn_func(data_port_FT)
    S = Spn(scat_port_FT, model_port_FT)
    C = Cdpn(data_port_FT, model_port_FT, scat_port_FT, phsr)
    chi2_prime = (nbin-1)*(np.log(Sdn - old_div(C ** 2, S))).sum()  # without data term Sd
    return chi2_prime

def loglikelihood(params, data_portrait_FT, model_portrait_FT,errs_FT, P, freqs, nu_DM, nu_GM, nu_tau, fit_flags, log10_tau):
    # print(Sd)
    # Sd, data_portrait_FT, model_portrait_FT,errs_FT, P, freqs, nu_DM, nu_GM, nu_tau, fit_flags, log10_tau = likelihood_args
    phi, DM, GM, tau, alpha = params
    #phi = param_dict["phi"]
    #DM = param_dict["DM"]
    #GM = 0.0
    ...
    S = Spn(scat_port_FT, model_port_FT)
    C = Cdpn(data_port_FT, model_port_FT, scat_port_FT, phsr)
    chi2_prime = (nbin-1)*(np.log(Sdn - old_div(C ** 2, S))).sum()
    # print("chi_prime: ",chi2_prime)
    # print("loglikelihood: ",-0.5*chi2_prime - 0.5*(  np.log(2*np.pi)*N + np.linalg.slogdet(covm)[0]*np.linalg.slogdet(covm)[1]))
    return -0.5*chi2_prime
...
def fit_portrait_full_function_deriv(params, data_portrait_FT,
    ...
    Sdn = Sdn_func(data_port_FT)
    S = Spn(scat_port_FT, model_port_FT)
    dS = Spn_deriv(abs_scat_port_FT_deriv, model_port_FT)
    C = Cdpn(data_port_FT, model_port_FT, scat_port_FT, phsr)
    dCdphi = Cdpn_deriv_phase_shifts(data_port_FT, model_port_FT, scat_port_FT,
                                     phsr)
    dC = Cdpn_deriv(data_port_FT, model_port_FT, scat_port_FT_deriv, phsr,
                    phis_deriv, dCdphi)
    den = (Sdn*S - C ** 2)
    num = -2*C*dC
    gradient = (nbin-1)*old_div(num, den).sum(axis=-1)
...
def fit_portrait_full_function_2deriv(params, data_portrait_FT,
    ...
    """
    avinash: Here becareful with the scales, we have not implemented the max likelihood noise
    """
    returns = []
    phi, DM, GM, tau, alpha = params
    if log10_tau: tau = 10 ** tau
    ...
    S = Spn(scat_port_FT, model_port_FT)
    dS = Spn_deriv(abs_scat_port_FT_deriv, model_port_FT)
    d2S = Spn_2deriv(abs_scat_port_FT_2deriv, model_port_FT)
    C = Cdpn(data_port_FT, model_port_FT, scat_port_FT, phsr)
    dCdphi = Cdpn_deriv_phase_shifts(data_port_FT, model_port_FT, scat_port_FT,
                                     phsr)
    d2Cdphi = Cdpn_2deriv_phase_shifts(data_port_FT, model_port_FT,
                                       scat_port_FT, phsr)
    dC = Cdpn_deriv(data_port_FT, model_port_FT, scat_port_FT_deriv, phsr,
                    phis_deriv, dCdphi)
    d2C = Cdpn_2deriv(data_port_FT, model_port_FT, scat_port_FT_deriv,
    ...
    for iparam in range(5):
        for jparam in range(5):  # repeating calculations even though symmetric
            ...
            den = (Sdn*S - C ** 2)
            num1 = -4*(C**2)*dC[iparam] * dC[jparam]
            num2 = -2*dC[iparam] * dC[jparam]
            num3 = -2*C*d2C[iparam, jparam]
            Hij_n = (nbin-1)*(old_div(num1,den**2)+old_div(num2,den)+old_div(num3,den))
...
def fit_portrait_full_function_2deriv_with_scales_band35(params, data_portrait_FT_band3, data_portrait_FT_band5,
    ...
    S_band3 = Spn(scat_port_FT_band3, model_port_FT_band3)
    dS_band3 = Spn_deriv(abs_scat_port_FT_deriv_band3, model_port_FT_band3)
    d2S_band3 = Spn_2deriv(abs_scat_port_FT_2deriv_band3, model_port_FT_band3)
    C_band3 = Cdpn(data_port_FT_band3, model_port_FT_band3, scat_port_FT_band3, phsr_band3)
    dCdphi_band3 = Cdpn_deriv_phase_shifts(data_port_FT_band3, model_port_FT_band3, scat_port_FT_band3,
                                     phsr_band3)
    d2Cdphi_band3 = Cdpn_2deriv_phase_shifts(data_port_FT_band3, model_port_FT_band3,
                                       scat_port_FT_band3, phsr_band3)
    dC_band3 = Cdpn_deriv(data_port_FT_band3, model_port_FT_band3, scat_port_FT_deriv_band3, phsr_band3,
                    phis_deriv_band3, dCdphi_band3)
    d2C_band3 = Cdpn_2deriv(data_port_FT_band3, model_port_FT_band3, scat_port_FT_deriv_band3,
                      scat_port_FT_2deriv_band3, phsr_band3, phis_deriv_band3, phis_2deriv_band3, dCdphi_band3,
                      d2Cdphi_band3)
    Sdn_band3 = Sdn_func(data_port_FT_band3)
    S_band5 = Spn(scat_port_FT_band5, model_port_FT_band5)
    dS_band5 = Spn_deriv(abs_scat_port_FT_deriv_band5, model_port_FT_band5)
    d2S_band5 = Spn_2deriv(abs_scat_port_FT_2deriv_band5, model_port_FT_band5)
    C_band5 = Cdpn(data_port_FT_band5, model_port_FT_band5, scat_port_FT_band5, phsr_band5)
    dCdphi_band5 = Cdpn_deriv_phase_shifts(data_port_FT_band5, model_port_FT_band5, scat_port_FT_band5,
                                     phsr_band5)
    d2Cdphi_band5 = Cdpn_2deriv_phase_shifts(data_port_FT_band5, model_port_FT_band5,
                                       scat_port_FT_band5, phsr_band5)
    dC_band5 = Cdpn_deriv(data_port_FT_band5, model_port_FT_band5, scat_port_FT_deriv_band5, phsr_band5,
                    phis_deriv_band5, dCdphi_band5)
    d2C_band5 = Cdpn_2deriv(data_port_FT_band5, model_port_FT_band5, scat_port_FT_deriv_band5,
                      scat_port_FT_2deriv_band5, phsr_band5, phis_deriv_band5, phis_2deriv_band5, dCdphi_band5,
                      d2Cdphi_band5)
    Sdn_band5 = Sdn_func(data_port_FT_band5)
    
    Sdn = np.concatenate((Sdn_band3,Sdn_band5),axis=0)
    ...
    nchan_band3 = data_portrait_FT_band3.shape[0]
    nchan_band5 = data_portrait_FT_band5.shape[0]
    nbin3 = nbin_band3*np.ones((nchan_band3,))
    nbin5 = nbin_band3*np.ones((nchan_band5,))
    nbin = np.concatenate((nbin3, nbin5))
    
    #print("Sdn",np.shape(Sdn))
    #print("S",np.shape(S))
    #print("dS",np.shape(dS))
    #print("d2S",np.shape(d2S))
    #print("C",np.shape(C))
    #print("dC",np.shape(dC))
    #print("d2C",np.shape(d2C))
    #print("dCdphi",np.shape(dCdphi))
    #print("d2Cdphi",np.shape(d2Cdphi))
    
    
    scales = old_div(C, S)  # maximum-likelihood values for amplitude parameters
    hessian = np.zeros([5 + len(freqs), 5 + len(freqs), len(freqs)])
    numa = (Sdn + (scales ** 2) * S)
    dena = (Sdn - (scales ** 2) * S)**2
    cross_hess = (-2 * nbin * dC)*(old_div(numa,dena))
    #print("cross_hess",np.shape(cross_hess))
    #print(ok)
    for iparam in range(5):
        for jparam in range(5):  # repeating calculations even though symmetric
            ...
            # num2 = -2*dC[iparam] * dC[jparam]
            num3 = -2*C*d2C[iparam, jparam]
            Hij_n = (nbin-1)*(old_div(num3,den))#avinash: not sure about this
    ...
    for ichan in range(len(freqs)):  # diagonal a_n matrix and cross terms
        ...
        Hij_n[ichan] = (2 * nbin[ichan] * S[ichan])*((old_div(numa[ichan],dena[ichan])))
    ...
    if return_covariance_matrix:
        ...
        C_inv = np.identity(len(scales)) * ((2 * nbin * S)*(old_div(numa,dena))) ** -1
        U = cross_hess[ifit]
        V = U.T
...
def get_noise_full(params, data_portrait_FT, model_portrait_FT, P,
                    freqs, nu_DM, nu_GM, nu_tau, log10_tau):
    """
    Return the maximum-likelihood, per-channel noise amplitudes.
    """
    phi, DM, GM, tau, alpha = params
    if log10_tau: tau = 10 ** tau
    data_port_FT = data_portrait_FT
    ...
    C = Cdpn(data_port_FT, model_port_FT, scat_port_FT, phsr)
    Sdn = Sdn_func(data_port_FT)
    noises = (old_div(2, nbin)*(Sdn - old_div(C**2,S)))**0.5
    return noises
...
def fit_portrait_full_function_2deriv_band35(params, data_portrait_FT_band3, data_portrait_FT_band5, 
    ...
    hessian_band3, scales_band3 = fit_portrait_full_function_2deriv(params_band3, data_portrait_FT_band3,
                                          model_portrait_FT_band3, errs_FT_band3, P, freqs_band3, nu_DM, nu_GM, nu_tau,
                                          fit_flags, log10_tau, per_channel=False,
                                          return_covariance_matrix=False, return_scales=True)
    hessian_band5, scales_band5 = fit_portrait_full_function_2deriv(params_band5, data_portrait_FT_band5,
                                          model_portrait_FT_band5, errs_FT_band5, P, freqs_band5, nu_DM, nu_GM, nu_tau,
                                          fit_flags, log10_tau, per_channel=False,
                                          return_covariance_matrix=False, return_scales=True)

    hessian_band35 = hessian_band3 + hessian_band5
    #print(np.shape(hessian_band35))
    #print(fit_flags)
    #print(ok)
    #print("hessian",hessian_band3)
    #print("hessian_band35",hessian_band35)
    returns = []
    #if not per_channel: hessian_band35 = hessian_band35.sum(axis=-1)
    returns.append(hessian_band35)
    if return_covariance_matrix:
        if per_channel: hessian_band35 = hessian_band35.sum(axis=-1)
        ifit = np.where(fit_flags)[0]  # which parameters are fit
        covariance_matrix = np.linalg.inv(0.5 * hessian_band35[ifit].T[ifit].T)
        returns.append(covariance_matrix)
    if return_scales:
        scales = np.concatenate((scales_band3,scales_band5),axis=0)
        returns.append(scales)
    if len(returns) == 1:
...
def loglikelihood_band35(params, data_portrait_FT_band3, data_portrait_FT_band5, 
                               model_portrait_FT_band3, model_portrait_FT_band5,
                               errs_FT_band3, errs_FT_band5, P, freqs_band3, freqs_band5, 
                               nu_DM, nu_GM, nu_tau, fit_flags, log10_tau, phase_offset, param_dict):
    #phi = param_dict["phi"]
    #DM = param_dict["DM"]
    #GM = 0.0
    #tau = 0.0
    ...
    params_band3 = np.copy(params)
    params_band5 = np.copy(params)
    params_band3[0]=param_dict["phi"]
    params_band5[0]=param_dict["phi"]+phase_offset
    params_band3[1]=param_dict["DM"]
    params_band5[1]=param_dict["DM"]

    chi2_prime_band3 = loglikelihood(params_band3, data_portrait_FT_band3, model_portrait_FT_band3,
                                   errs_FT_band3, P, freqs_band3, nu_DM, nu_GM, nu_tau, fit_flags, log10_tau)
    chi2_prime_band5 = loglikelihood(params_band5, data_portrait_FT_band5, model_portrait_FT_band5,
                                   errs_FT_band5, P, freqs_band5, nu_DM, nu_GM, nu_tau, fit_flags, log10_tau)
    chi2_prime_band35 = chi2_prime_band3 + chi2_prime_band5
...
def fit_portrait_full_mlan(data_port_band3, data_port_band5, model_port_band3, model_port_band5,
                      ...
                      method='trust-ncg', is_toa=True, quiet=True, bayes=False, filename = None, pool = 10, outdir=None):
    #print(nu_fits)
...
# =============================================================================
    ...
    Hij, covariance_matrix, scales = \
        fit_portrait_full_function_2deriv_band35(params,
                                                      data_port_FT_band3, data_port_FT_band5, 
                                                      model_port_FT_band3, model_port_FT_band5,
                                                      errs_FT_band3, errs_FT_band5, P, freqs_band3, freqs_band5,
    ...
    param_errs[ifit] = all_param_errs[:nfit]
    ...
    phi_inf = phase_shifts(phi_out, DM_fit, GM_fit, np.inf, nu_out_DM,
                           nu_fit_GM, P, False)
    phi_err=param_errs[0]
    DM_err=param_errs[1]
    phi_DM_err = phase_shifts(0, DM_err, GM_fit, np.inf, nu_out_DM,
                           nu_out_GM, P, False)
    phi_inf_err = np.sqrt(phi_err**2 + phi_DM_err**2)
    
    if bayes:
        import nautilus
        import corner
        other_args_bayes = (params, data_port_FT_band3, data_port_FT_band5, model_port_FT_band3, model_port_FT_band5,
                    errs_FT_band3, errs_FT_band5, P, freqs_band3, freqs_band5, nu_out_DM,
                    nu_out_GM, nu_out_tau, list(map(bool, fit_flags)), log10_tau, phase_offset)
        
        from scipy.stats import norm
        from nautilus import Prior

        prior = Prior()
        prior.add_parameter('phi', dist=(phi_out-0.5, phi_out+0.5))
        phi_DM1 = phase_shifts(0, 1, GM_fit, 300, 500,
                            nu_out_GM, P, False)
        dDM1 = 0.1/phi_DM1
        prior.add_parameter('DM', dist=(DM_fit-dDM1, DM_fit+dDM1))

        from nautilus import Sampler
        sampler = Sampler(prior, loglikelihood_band35, n_live=1000, pool=int(pool), resume=False, likelihood_args=other_args_bayes)
        sampler.run(discard_exploration=True,verbose=True)
        
        points, log_w, log_l = sampler.posterior()
        
        prior_keys = prior.keys
        prior_keys.append("phi_inf")
        points = np.concatenate([points, np.zeros((points.shape[0], 1))], axis=1)
        points.T[2]=phase_shifts(points.T[0], points.T[1], GM_fit, np.inf, nu_out_DM,
                            nu_out_GM, P, False)
        fit_values = [phi_out, DM_fit,phi_inf]
        fit_value_errs = np.array([phi_err, DM_err, phi_inf_err])
        ndim = points.shape[1]

        fig, axes = plt.subplots(ndim, ndim, figsize=(10, 10))
        figure = corner.corner(points, weights=np.exp(log_w), bins=20, labels=prior_keys,
        ...
        # print("prior_keys:", prior_keys)
        param_stats = {}
        for i, param_name in enumerate(prior_keys):
            q_16, q_50, q_84 = quantile(points[:, i], [0.16, 0.5, 0.84], weights=np.exp(log_w))
        ...
        from mpp_gaussian_test import gaussian_test
        ...
        phi_inf_final = param_stats["phi_inf"]["median"]
        DM_final = param_stats["DM"]["median"]
        phi_err_final = param_stats["phi"]["error"]
        DM_err_final = param_stats["DM"]["error"]
        phi_inf_err_final = param_stats["phi_inf"]["error"]
        log_z_final = sampler.log_z
        print("Its new algorithm mlan")
        ########################################################################################
        fit_port_results = DataBunch(
                                    # params=params, param_errs=param_errs,
                                    # phase=phi_out, phase_err=param_errs[0], DM=DM_fit,
                                    phi=phi_final, phi_err=phi_err_final, DM=DM_final,
                                    DM_err=DM_err_final, GM=GM_fit, GM_err=param_errs[2],
                                    ...
                                    phi_inf=phi_inf_final, phi_inf_err=phi_inf_err_final, log_z=log_z_final, gt=gt,
