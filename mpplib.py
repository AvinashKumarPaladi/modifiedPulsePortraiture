...
from mpp_telescope_codes import telescope_code_dict
...
def load_data(filename, state=None, dedisperse=False, dededisperse=False,
    ...
    data = DataBunch(arch=arch, backend=backend, backend_delay=backend_delay,
                     ...
                     telescope_code=telescope_code, weights=weights, beconfig=beconfig)
    return data
    ...
    for toa in toas:
        ...
        for flag, value in list(toa.flags.items()):
            if value is not None:
                ...
                elif flag.find("epoch") >= 0:
                    toa_string += ' -%s %s' % (flag, toa.flags[flag])
                elif flag.find("TOA") >= 0:
                    toa_string += ' -%s %s' % (flag, toa.flags[flag])
                elif flag.find("P") >= 0:
                    toa_string += ' -%s %.30f' % (flag, toa.flags[flag])
                elif flag.find("be_delay") >= 0:
                    toa_string += ' -%s %.15f' % (flag, toa.flags[flag])
                elif flag.find("phi") >= 0:
                    toa_string += ' -%s %.8f' % (flag, toa.flags[flag])
                elif flag.find("DM") >= 0:
                    toa_string += ' -%s %.8f' % (flag, toa.flags[flag])
                else:
