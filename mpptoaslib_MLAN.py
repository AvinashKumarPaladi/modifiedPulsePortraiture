...
# def loglikelihood(likelihood_args,param_dict):
#     y1data = likelihood_args
#     x = param_dict['A']*np.cos(((2*np.pi)/param_dict['P'])*xdata + param_dict['phi'])+param_dict['S0']
#     covm=np.diag((yerr**2)*(param_dict['EFAC']**2))
#     DS = y1data - x
#     normal_exponent = np.dot(DS,scipy.linalg.pinvh(covm))
#     normal_exponent = np.dot(normal_exponent,DS.T)
#     N = 100
#     return -0.5*normal_exponent - 0.5*(  np.log(2*np.pi)*N + np.linalg.slogdet(covm)[0]*np.linalg.slogdet(covm)[1])
...
#     chi2_prime = Sd-(old_div(C ** 2, S)).sum()
#     print("chi_prime: ",chi2_prime)
#     return chi2_prime

def loglikelihood(data_portrait_FT, model_portrait_FT,errs_FT, P, freqs, nu_DM, nu_GM, nu_tau, fit_flags, log10_tau,param_dict):
    # print(Sd)
    # Sd, data_portrait_FT, model_portrait_FT,errs_FT, P, freqs, nu_DM, nu_GM, nu_tau, fit_flags, log10_tau = likelihood_args
    phi = param_dict["phi"]
    DM = param_dict["DM"]
...
def fit_portrait_full_function_2deriv_with_scales(params, data_portrait_FT,
    ...
    for iparam in range(5):
        for jparam in range(5):  # repeating calculations even though symmetric
            ...
            # num3 = -2*C*d2C[iparam, jparam]
            # Hij_n = (nbin-1)*(old_div(num1,den**2)+old_div(num2,den)+old_div(num3,den))
            # Hij_n = (nbin-1)*(old_div(num1,den**2)+old_div(num2,den))
            # print("Hij_n", Hij_n)
            # print(ok)
    ...
    for ichan in range(len(freqs)):  # diagonal a_n matrix and cross terms
        ...
        Hij_n[ichan] = (2 * nbin * S[ichan])*((old_div(numa[ichan],dena[ichan])))
        # Hij_n[ichan] = 2 * S[ichan] #avinash: not sure about this
...
def fit_portrait_full_mlan(data_port, model_port, init_params, P, freqs,
    ...
    if errs is None:
        errs_FT = get_noise_full(params, data_port_FT, model_port_FT, P,
                    freqs, nu_fit_DM, nu_fit_GM, nu_fit_tau, log10_tau)
    ...
    # }
    # # print(loglikelihood(li,ydata))
    # # print(likelihood(li))
    # # print()
    # # print(loglikelihood(other_args_bayes,li))

    # from functools import partial
    # likelihood_args=other_args_bayes
    # lilikelihood = partial(loglikelihood,*likelihood_args)
    # print(lilikelihood(li))
    # print(ok)
    ...
    Hij, covariance_matrix, scales = \
        fit_portrait_full_function_2deriv(params,
                                                      data_port_FT, model_port_FT, errs_FT, P, freqs, nu_out_DM,
                                                      nu_out_GM, nu_out_tau, fit_flags, log10_tau,
                                                      per_channel=False, return_covariance_matrix=True,
                                                      return_scales=True)
    ...
    # print("phi_inf_err:", phi_inf_err)
    # print("GM:", GM_fit)
    # print("phi_DM_err:", phi_DM_err)
    ...
    if bayes:
        import nautilus
        import corner
        other_args_bayes = (data_port_FT, model_port_FT, errs_FT, P, freqs, nu_out_DM,
                  nu_out_GM, nu_out_tau, list(map(bool, fit_flags)), log10_tau)
        ...
        sampler = Sampler(prior, loglikelihood, n_live=1000, pool=int(pool), resume=False, likelihood_args=other_args_bayes)
        sampler.run(discard_exploration=True,verbose=True)

        # points, log_w, log_l = sampler.posterior()
        # ndim = points.shape[1]
        # prior_keys = prior.keys[:ndim]
