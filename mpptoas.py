...
class GetTOAs(object):
    ...
    def get_TOAs(self, datafile=None, tscrunch=False, nu_refs=None, DM0=None,
        ...
        for iarch, datafile in enumerate(datafiles):
            ...
            for isub in d.ok_isubs:
                ...
                results.TOA_inf = epoch + pr.MJD(
                    old_div(((results.phi_inf * P) + d.backend_delay),
                            (3600 * 24.)))
                results.TOA_inf_err = results.phi_inf_err * P * 1e6  # [us]
                ...
                toa_flags['pta'] = "InPTA"
                ar_mjd = epoch.intday()
                ar_freq = np.mean(d.freqs)
                ar_bec = d.beconfig
                if (ar_mjd < 58600.):
                    toa_flags['group'] = 'GM_GWB_'+ar_bec[1]+'_'+ar_bec[2]+'_'+ar_bec[0]+'_pre36'
                    toa_flags['sys'] = 'GM_GWB_'+ar_bec[1]+'_'+ar_bec[2]+'_'+ar_bec[0]
                if (ar_mjd > 58600.):
                    toa_flags['group'] = 'GM_GWB_'+ar_bec[1]+'_'+ar_bec[2]+'_'+ar_bec[0]+'_post36'
                    toa_flags['sys'] = 'GM_GWB_'+ar_bec[1]+'_'+ar_bec[2]+'_'+ar_bec[0]
                ...
                if print_phase:
                    ...
                    toa_flags['phi_DM_cov'] = \
                        results.covariance_matrix[0, 1]
                    toa_flags['epoch'] = "%d" % (epoch.intday()) + ("%.15f" % (epoch.fracday()))[1:]
                    toa_flags['be_delay'] = d.backend_delay
                    toa_flags['P'] = P
                if bayes:
                ...
                if bayes and mlan:
                    toa_flags['technique'] = "MLAN_bayes"
                if not bayes and mlan:
                    toa_flags['technique'] = "MLAN_freq"
                if bayes and not mlan:
                    toa_flags['technique'] = "MLA_bayes"
                if not bayes and not mlan:
                    toa_flags['technique'] = "MLA_freq"
            ...
            if show_plot:
                #print(f"DEBUG: show_plot = {show_plot}")  # Add this line
                stop = time.time()
                ...
                for isub in d.ok_isubs:
                    if show_plot=="save":
                        ...
                        savefig = False
                        # show_display = True   # Display when not saving
    ...
    def show_fit(self, datafile=None, isub=0, rotate=0.0, show=True,
        ...
        if show:
            #print(f"DEBUG: Calling show_residual_plot with savefig={savefig}")  # Add this line
